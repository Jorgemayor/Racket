    ...
    (cases expression exp
      ...
      (super-exp (idMethod rands)
                 (let ((args (eval-rands rands env exps))
                   ...
                   (find-method-and-apply
                    (string->symbol idMethod) (apply-env env '%super) obj args)))
      )
    )
  )
...
(define class-decl->super-name
  (lambda (c-decl)
    (cases class-decl c-decl
      (a-class (class-name super-name field-ids m-decls)
               (string->symbol super-name)))))

(define class-decl->field-ids
  (lambda (c-decl)
    (cases class-decl c-decl
      (a-class (class-name super-name field-ids m-decls)
               (listOfString->listOfSymbols field-ids)))))
...
(define method-decl->ids
  (lambda (md)
    (cases method-decl md
      (a-method (method-name ids body)
                (listOfString->listOfSymbols ids)))))
...
(define method-decls->method-names
  (lambda (mds)
    (listOfString->listOfSymbols (map method-decl->method-name mds))))
...
(define view-object-as
  (lambda (parts class-name)
    (cond [(eqv? (part->class-name (car parts)) class-name) parts]
          [else (view-object-as (cdr parts) class-name)])))
  ...
  (lambda (name)
    (let loop ((env the-class-env))
      (cond
        ...
        ((eq? (class-decl->class-name (car env)) name) (car env))
        (else (loop (cdr env)))))))
