...
;            -> {list}
;Purpose:
;Returns a list with an 'empty-tree.
(define empty-tree
  (lambda ()
    (list 'empty-tree)
    )
...
(define tree1 (list 5 (empty-tree)
                    (empty-tree)))

(define tree2 (list 5 (list 9 (empty-tree)
                            (list 5 (empty-tree)
                                  (empty-tree)))
...
(define tree3 (list 5 (list 1 (list 0 (empty-tree)
                                    (empty-tree))
                            (list 3 (empty-tree)
                                  (empty-tree)))
                    (list 9 (empty-tree)
...
;            -> {boolean}
;Purpose:
;Returns true if the tree given is an empty-tree, false otherwise.
(define empty-tree?
  (lambda (tree)
...
(make-tree 4 (make-tree 3 (empty-tree) (empty-tree))
...
;              -> {int}
;Purpose:
;Extracts the value (node) of a tree given.
(define extract-node
  (lambda (tree)
    (car tree)
    )
...
;Pruebas
(extract-node (make-tree 4 (empty-tree) (empty-tree)))
(extract-node tree1)
...
;             -> {list}
;Purpose:
;Extracts the left tree of a tree given.
(define extract-izq
  (lambda (tree)
    (cadr tree)
    )
...
;             -> {list}
;Purpose:
;Extracts the right tree of a tree given.
(define extract-der
  (lambda (tree)
    (caddr tree)
    )
...
;           -> {list}
;Purpose:
;Returns true if the tree given is a leaf, false
;otherwise.
;An empty-tree is not taken as a leaf.
(define leaf-tree?
  (lambda (tree)
...
;            -> {boolean}
;Purpose:
;Returns true if the tree given is a node, false
;otherwise.
...
(define insertar-elemento
  (lambda (tree number)
    (letrec ([insertar (lambda (tree)
                         (cond [(empty-tree? tree) (make-tree number (empty-tree) (empty-tree))]
